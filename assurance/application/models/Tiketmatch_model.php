...
class TiketMatch_model extends CI_Model {
    ...
    public function getTiketArea($area) {
        ...
        return $this->db->query("SELECT * FROM nossa WHERE `status` <> `status_tiket` AND sektor = '$area' AND status <> 'CLOSED' ")->result();
    ...
    public function getTiketAll() {
        ...
        $this->db->query("SELECT * FROM nossa WHERE (status <> status_tiket AND status <> 'CLOSED')")->result();
